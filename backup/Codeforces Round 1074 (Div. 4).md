...
- 如果 `a_i <= pre`，它必须消耗一次作弊机会才能赢下这一轮（相当于赢下这“一堆”牛），技能值也变为 `a_i + pre`。
...
- **查询**：对于每头牛，二分查找临界点 `O(log n)`，然后遍历 `q` 列表进行检查。
  - 虽然看起来是 `O(n * |q|)`，但由于技能值呈指数级增长，“强者”的数量 `|q|` 非常少（最多 `O(log(sum(a_i)))`，约为 60 个）。
- **总复杂度**：`O(n * log(TotalSum))`，在 `2 * 10^5` 范围内非常快。
